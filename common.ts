...
import { readableStreamToString, WorkdayPersonError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://<tenantAuthorizationHostname>"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new WorkdayPersonError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new WorkdayPersonError(e, e.response?.data)
                }
                if (innerError instanceof WorkdayPersonError) {
                    // Got "WorkdayPersonError" from the above try block
                    throw innerError;
                }
